...
for i in range((len(surr.model.layers)-1)):
    first_layer_nn = surr.model.get_layer(index=i)(first_layer_nn)

last_layer_nn = surr.model.get_layer(index=-1)(first_layer_nn)
output_layer_nn = last_layer_nn

final_model_nn = keras.models.Model(inputs=[inp_layer_nn], outputs=[output_layer_nn])
final_model_nn.layers[-1].trainable = True
...
output_layer_pinn = keras.layers.concatenate([inp_layer_pinn, first_layer_pinn])

final_model_pinn = keras.models.Model(inputs=[inp_layer_pinn], outputs=[output_layer_pinn])
final_model_pinn.layers[-1].trainable = True
